class Users::AnalysisController < ApplicationController
  def show
    ...
    # グラフ作成のための配列
    @all_tasks = {"未着手" => off_tasks.count, "実行中" => @on_task.count, "終了済" => @finished_tasks.count}
    # ルームのタスクを全て合わせたもの
  ...
  def edit
    ...
    @on_task = Task.where(room_id: current_user.present_room, user_id: @room_user.user_id, status: 1)
    @finished_tasks = Task.where(room_id: current_user.present_room, user_id: @room_user.user_id, status: 2)
  end
end
